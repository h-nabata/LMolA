...
def run_workflow_yaml(workflow_yaml_path: str) -> WorkflowExecutionResult:
    ...
    for idx, item in enumerate(item_inputs, start=1):
        ...
        current_structure: str | None = item["value"] if req.input.type in {"xyz", "xyz_list"} else None
        try:
            for step in steps:
                ...
                if step_tool.startswith("generate_small_molecule"):
                    ...
                    step_params.setdefault("structure_path", current_structure)
                elif step_tool == "relax_structure_xtb":
                    step_params.setdefault("input_structure", current_structure)
                ...
                if step_tool.startswith("generate_small_molecule"):
                    ...
                    current_structure = out.payload.get("primary_structure")
                    r.primary_structure = current_structure
                    if out.payload.get("generated_files"):
                        for gf in out.payload["generated_files"]:
                            if str(gf).endswith(".sdf"):
                                r.conformer_ensemble_path = gf
                elif step_tool == "validate_structure_ase":
                    r.validation_status = out.status
                    r.validation_report_path = str(step_run_dir / "validation_report.json")
                elif step_tool == "relax_structure_xtb":
                    r.relax_status = out.status
                    r.relax_run_dir = out.run_dir
                    rel = out.payload.get("output_structure")
                    if rel:
                        r.relaxed_structure = str(Path(out.run_dir) / rel)
                    tool_exec = Path(step_run_dir / "tool_execution_result.json")
                    if tool_exec.exists():
    ...
    summary = WorkflowSummary(batch_id=batch_id, workflow_id=req.workflow_id, item_count=len(results), ok_count=ok_count, error_count=len(results) - ok_count)
    dump_json(batch_dir / "workflow_result.json", {"status": "ok", "message": "Workflow executed", "summary": summary.model_dump()})
    (batch_dir / "run.log").write_text("Workflow executed deterministically.\n", encoding="utf-8")
    (batch_dir / "README_batch.md").write_text("# LMolA Batch Workflow Run\n", encoding="utf-8")

    return WorkflowExecutionResult(status="ok", message="Workflow executed", batch_dir=str(batch_dir), summary_csv=str(batch_dir / "summary.csv"), summary_json=str(batch_dir / "summary.json"), summary=summary)
