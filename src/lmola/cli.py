...
from lmola.dry_run_plan import create_dry_run_execution_plan, run_dry_run_plan_eval, run_existing_tool_expansion_eval, run_morfeus_pilot_eval, run_molsimplify_pilot_eval, run_phase17_existing_tool_depth_eval
from lmola.adapters import list_adapter_metadata, run_optional_smoke_check
...
@artifact_app.command("export-contracts")
def artifact_export_contracts(fmt: str = typer.Option("json", "--format")) -> None:
    payload = {"status": "ok", **export_artifact_registry(compact=False)}
    if fmt != "json":
        raise typer.BadParameter("Only --format json is currently supported.")
    typer.echo(json.dumps(payload, indent=2, sort_keys=True))
...
@mcp_app.command("parameter-binding-smoke")
def mcp_parameter_binding_smoke(backend: str = typer.Option("mock", "--backend"), model: str = typer.Option("", "--model"), base_url: str = typer.Option("http://127.0.0.1:11434", "--base-url"), temperature: float = typer.Option(0.0, "--temperature"), timeout_seconds: int = typer.Option(20, "--timeout-seconds"), max_tokens: int = typer.Option(800, "--max-tokens"), cases: str = typer.Option("examples/phase16_1_parameter_binding_cases.yaml", "--cases"), fmt: str = typer.Option("json", "--format")) -> None:
    result = run_parameter_binding_eval(cases, backend=backend, model=model, base_url=base_url, temperature=temperature, timeout_seconds=timeout_seconds, max_tokens=max_tokens)
    print(json.dumps(result, indent=2) if fmt == "json" else result)
    if result.get("status") != "ok":
        raise typer.Exit(code=1)
...
@mcp_app.command("llm-execution-smoke", help="Run LLM execution smoke checks; --execute-safe enables safe mock execution cases.")
...
@mcp_app.command("llm-orchestration-smoke", help="Run LLM orchestration smoke checks; --execute-safe enables safe mock execution cases.")
def mcp_llm_orchestration_smoke(
    backend: str = typer.Option("mock", "--backend"),
    model: str = typer.Option("", "--model"),
    base_url: str = typer.Option("http://127.0.0.1:11434", "--base-url"),
    temperature: float = typer.Option(0.0, "--temperature"),
...
@backends_app.command("list-adapters")
def backends_list_adapters(fmt: str = typer.Option("json", "--format")) -> None:
    payload = {k: v.model_dump() for k, v in list_adapter_metadata().items()}
    _emit_schema(payload, fmt)


@backends_app.command("smoke")
def backends_smoke(backend: str = typer.Option(..., "--backend"), fmt: str = typer.Option("json", "--format")) -> None:
    try:
        payload = run_optional_smoke_check(backend).model_dump()
    except KeyError:
        typer.echo(json.dumps({"status": "error", "error_type": "unknown_smoke_backend", "backend": backend}, indent=2, sort_keys=True))
        raise typer.Exit(code=1)
    _emit_schema(payload, fmt)
...
@workflow_app.command("eval-existing-tools")
def workflow_eval_existing_tools(cases: str = typer.Argument(...), backend: str = typer.Option("mock", "--backend"), model: str = typer.Option("", "--model"), base_url: str = typer.Option("http://127.0.0.1:11434", "--base-url"), temperature: float = typer.Option(0.0, "--temperature"), timeout_seconds: int = typer.Option(20, "--timeout-seconds"), max_tokens: int = typer.Option(800, "--max-tokens"), fmt: str = typer.Option("json", "--format")) -> None:
    if "phase17" in Path(cases).name:
        result = run_phase17_existing_tool_depth_eval(cases, backend=backend, model=model, base_url=base_url, temperature=temperature, timeout_seconds=timeout_seconds, max_tokens=max_tokens)
    else:
...
@workflow_app.command("eval-morfeus-pilot")
def workflow_eval_morfeus_pilot(cases: str = typer.Argument(...), backend: str = typer.Option("mock", "--backend"), model: str = typer.Option("", "--model"), base_url: str = typer.Option("http://127.0.0.1:11434", "--base-url"), temperature: float = typer.Option(0.0, "--temperature"), timeout_seconds: int = typer.Option(20, "--timeout-seconds"), max_tokens: int = typer.Option(800, "--max-tokens"), fmt: str = typer.Option("json", "--format")) -> None:
    result = run_morfeus_pilot_eval(cases, backend=backend, model=model, base_url=base_url, temperature=temperature, timeout_seconds=timeout_seconds, max_tokens=max_tokens)
    typer.echo(json.dumps(result, indent=2, sort_keys=True) if fmt == "json" else result)
    if result.get("status") != "ok":
        raise typer.Exit(code=1)
...
def workflow_bind_parameters(prompt: str = typer.Option(..., "--prompt"), language: str = typer.Option("auto", "--language"), fmt: str = typer.Option("json", "--format")) -> None:
    ...
    print(json.dumps(payload, indent=2) if fmt == "json" else payload)


@workflow_app.command("eval-parameter-binding")
def workflow_eval_parameter_binding(cases_yaml: str, backend: str = typer.Option("mock", "--backend"), model: str = typer.Option("", "--model"), base_url: str = typer.Option("http://127.0.0.1:11434", "--base-url"), temperature: float = typer.Option(0.0, "--temperature"), timeout_seconds: int = typer.Option(20, "--timeout-seconds"), max_tokens: int = typer.Option(800, "--max-tokens"), fmt: str = typer.Option("json", "--format")) -> None:
    result = run_parameter_binding_eval(cases_yaml, backend=backend, model=model, base_url=base_url, temperature=temperature, timeout_seconds=timeout_seconds, max_tokens=max_tokens)
    print(json.dumps(result, indent=2) if fmt == "json" else result)
    if result.get("status") != "ok":
        raise typer.Exit(code=1)
...
@mcp_app.command("morfeus-pilot-smoke")
def mcp_morfeus_pilot_smoke(backend: str = typer.Option("mock", "--backend"), model: str = typer.Option("", "--model"), base_url: str = typer.Option("http://127.0.0.1:11434", "--base-url"), temperature: float = typer.Option(0.0, "--temperature"), timeout_seconds: int = typer.Option(20, "--timeout-seconds"), max_tokens: int = typer.Option(800, "--max-tokens"), cases: str = typer.Option("examples/phase16_5_morfeus_pilot_cases.yaml", "--cases"), fmt: str = typer.Option("json", "--format")) -> None:
    result = run_morfeus_pilot_eval(cases, backend=backend, model=model, base_url=base_url, temperature=temperature, timeout_seconds=timeout_seconds, max_tokens=max_tokens)
    typer.echo(json.dumps(result, indent=2, sort_keys=True) if fmt == "json" else result)
    if result.get("status") != "ok":
        raise typer.Exit(code=1)
